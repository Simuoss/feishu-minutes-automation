...
class R2SyncStateRepository:
    ...
    ) -> R2SyncStateEntity | None:
        stmt = select(R2SyncStateORM).where(
            R2SyncStateORM.owner_user_id == owner_user_id,
            R2SyncStateORM.minute_token == minute_token,
        )
    ...
    async def upsert(self, entity: R2SyncStateUpsertEntity) -> R2SyncStateEntity:
        stmt = select(R2SyncStateORM).where(
            R2SyncStateORM.owner_user_id == entity.owner_user_id,
